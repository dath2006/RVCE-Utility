  ...
  const unsubscribe = onValue(
    ...
    (snapshot) => {
      if (snapshot.exists()) {
        ...
        console.log(
          "No data available at 'stats' node. Will use default stats until seeded.",
        );
        callback({
          totalVisits: 14502,
          verifiedUsers: 1432,
...
export const submitRedesignFeedback = async (payload) => {
  const feedbackRef = ref(database, "feedback/redesign");

  const safePayload = {
    rating: Number(payload?.rating) || 0,
    suggestion: (payload?.suggestion || "").toString().slice(0, 1000),
